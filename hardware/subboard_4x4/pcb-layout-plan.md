...
- 板尺寸：**43.46 × 23.81 mm**（`PCB1`，实测值；`1 mm = 39.37 单位`）
  > 📐 来源（两处交叉验证一致）：`single-channel.fzz` 内 `single-channel.fz` 的 `<board width="4.34588cm" height="2.38071cm">`；`gerber-rs-274x/single-channel_contour.gm1` D10 aperture `R,1.710980X0.937287`（英寸）≈ 43.46 × 23.81 mm。
  > ⚠️ 早期规划曾写 84.67 × 56.44 mm，与实际 fzz/gerber 不符，已按实测值修正（2026-08-31）。
- 元件清单（PCB 视图共 9 个实体 + 8 个 NetLabel-Pad 过孔焊盘接口）：
  | 位号 | 元件 | 封装 | 当前层 |
  |---|---|---|---|
...
> 说明：线圈放左、逐级向右，信号流一目了然；U3 靠右贴近将来接控制板的 8 网络标签方向（网络标签用自建 `NetLabel-Pad` 已在 PCB 生成实体过孔焊盘 PAD-1.2-H0.6 ×8，实测见 PnP）。
