...
- **交叉**：**不需要** ✓ —— 两口**同名针在板上接到同一个 MCU 脚** ✓ ⇒ **怎么插都通** ✓（拿哪根线、插哪个口都对 ✓）
  ⚠️ 但**别理解成“全网并联总线”** ✗ —— 拓扑是**链**：段与段靠每块中继 ✓ ⇒ **断一块、下游全灭** ✗（见 [`bus-protocol.md`](bus-protocol.md) §2 ✓）
- **防呆**：做到最强 ✓✓ —— 两口完全等价，**不存在"插到 OUT 上"这种错误** ✓；
  3P 座本身有极性 ✓，插头也插不反 ✓

⚠️ **代价（必须认下来）**：这就是 `README §5.2` 的 **3 线方案**，固件要**寻址/枚举**（文档里标的
"⚠️ 需一次性标定" ✓）；并且原 **4 线方案**的"4 网"作废 —— `DATA_IN` / `DATA_OUT` 合成一根，
...
> 类比：4 线方案是**电话**（两根线、双方同时说 ✓）；3 线方案是**对讲机**（一个频道、轮流说 ✓）。
> 3 线方案比 4 线方案少掉的那根，就是**回程数据线** —— 这就是"4 网作废"的意思 ✓。
...
- 回读：帧里带"回读时隙"，像素在**自己的时隙**里驱动总线（**开漏 + 上拉**最省事 ✓），
  或走**令牌轮询** ✓
- 上电**枚举 / 一次性标定**：给每块分配地址并存进 Flash ✓（3 线方案的已知代价 ✓）
