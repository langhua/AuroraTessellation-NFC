...
|---|---|---|---|---|
| **A1** LED 硬件直穿链 + 传感链 | 6 根：5V/GND/**LED_IN/LED_OUT**/SENS_IN/SENS_OUT | 4/18 | ✅ 天然 | LED 自己再生（最稳） |
| **A2**（原定）一条链跑「颜色下行 + 读数下行」，MCU 自驱 LED | **4 根**（DATA_IN/OUT 分开）—— **2026-09-25 已合并进 B** ✗ | 5/18 | ✅ 天然 | MCU 产生（**SPI + DMA** ✓）|
| **B** ✅ **采用** 共享总线轮询，MCU 自驱 LED | **3 根**：5V/GND/**DATA**（半双工）| 4/18 | ✅ 链序 = 物理序（上电枚举一次 `index` ✓）| MCU 产生（**SPI + DMA** ✓）|

- **A1 为什么有 LED_IN/LED_OUT**：WS2812B 是**硬件**菊花链，链要在每块板中间「断开」接进这一颗 1010
  （本板 1010 的 DIN 接上游、DOUT 接下游）⇒ 接插件必须引出这两根。
  它们不是额外功能线，**就是 WS2812B 链本身穿过这块板**。
- **A2 / B（✅ 采用的是 B 的接线）**：LED 不串硬件链，而是每块 MCU 自己驱动自己那颗 1010；颜色随链下发，
  ...
  **与 A2 的唯一区别**：收发**共用一根 `DATA`**（半双工，各节点只在自己的时隙驱动 ✓）
  ⇒ 接插件从 4 根降到 **3 根**，而且**两口完全等价、可正反插** ✓（见 [`connector-plan.md`](connector-plan.md) ✓）。

> ✅ **已定（2026-09-25）：方案 B —— 板间 3 根线（5V / GND（**居中**）/ **DATA**）** ✓
> MCU 自驱 LED，一条链同时跑「颜色下行 + 读数回报」；**单向半双工**：收发共用 `DATA`，
> 每个节点只在自己的时隙驱动 ✓；**两口并联、完全等价** ⇒ 正反插都对 ✓（见 [`connector-plan.md`](connector-plan.md) ✓）。
> A2（4 根：`DATA_IN` / `DATA_OUT` 分开）**已合并进 B** ✓ —— 总线本来就是半双工，多一根是白送线 ✓。
> A1（带 LED_IN/LED_OUT 的硬件链）作为「零 LED 时序风险」备选保留。

单独销售形态：单块 5 V 上电即工作（固件内置局部归一化）；串联时用 **3 线**对接（`MX-1.25-3P-V` 立贴，两口等价 ⇒ 随便插 ✓）。
...
**不做参数测试板（coupon）了** —— 直接打像素板、用 A/B 两板对比（详见 [`coupon-layout.md`](coupon-layout.md)）：
