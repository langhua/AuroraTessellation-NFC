...
| **4 线方案**（原定，已废弃 ✗）一条链跑「颜色下行 + 读数下行」，MCU 自驱 LED | **4 根**（DATA_IN/OUT 分开）—— **2026-09-25 已合并进 3 线方案** ✗ | 5/18 | ✅ 天然 | MCU 产生（**SPI + DMA** ✓）|
| **3 线方案** ✅ **采用** 共享总线轮询，MCU 自驱 LED | **3 根**：5V/GND/**DATA**（半双工）| 4/18 | ✅ 链序 = 物理序（上电枚举一次 `index` ✓）| MCU 产生（**SPI + DMA** ✓）|
...
- **4 线 / 3 线方案（✅ 采用的是 3 线方案的接线）**：LED 不串硬件链，而是每块 MCU 自己驱动自己那颗 1010；颜色随链下发，
  ...
  **与 4 线方案的唯一区别**：收发**共用一根 `DATA`**（半双工，各节点只在自己的时隙驱动 ✓）
  ⇒ 接插件从 4 根降到 **3 根**，而且**两口完全等价、可正反插** ✓（见 [`connector-plan.md`](connector-plan.md) ✓）。

> ✅ **已定（2026-09-25）：3 线方案 —— 板间 3 根线（5V / GND（**居中**）/ **DATA**）** ✓
> MCU 自驱 LED，一条链同时跑「颜色下行 + 读数回报」；**单向半双工**：收发共用 `DATA`，
> 每个节点只在自己的时隙驱动 ✓；**两口同名针在板上并接、完全等价** ⇒ 正反插都对 ✓
> （拓扑是**链**：段与段靠每块中继 ✓ ⇒ **断一块、下游全灭** ✗ —— 见 [`bus-protocol.md`](bus-protocol.md) §2 ✓）。
> **4 线方案**（`DATA_IN` / `DATA_OUT` 分开）**已合并进 3 线方案** ✓ —— 总线本来就是半双工，多一根是白送线 ✓。
