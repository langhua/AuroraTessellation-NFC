...
| 12 | `C2.1` | `5V` | 去耦 |
| 13 | `C2.2` | `GND` | 去耦 |
| 14 | `U1.pin3`（**PA2**）| `DATA` | 链 DATA：**单根半双工** ✓，两口等价 ✓ |
| 15 | — | — | 原 `DATA_OUT` **已合并进第 14 行**（2026-09-25 定三线）✗ |
| 16 | `U1.pin13`（**PC6 / `SPI_MOSI`**）| `LED1.DIN` | 自驱本板 1010 ✓（固件走 SPI + DMA ✓）|
| 17 | `LED1.VDD` | `5V` | |
| 18 | `LED1.VSS` | `GND` | |
| 19 | `U1.pin0`（**EPAD**）| `GND` | ⚠️ **独立成网** —— 布线时**特意**接到 GND ✓（别指望自动合并 ✗）|

**不接**：`LED1.DOUT`（LED 不在硬件链上，链在 MCU 之间 ✓）；`U1.pin15`（**PD1 = SWIO**）留个测试点、平时悬空 ✓；其余 13 个备用脚（PD7 / PD0 / PC0–PC5 / PC7 / PD2–PD6）也留测试点或悬空 ✓。
...
- 接插件针序 **`5V` / `GND`（居中）/ `DATA`**，**两口完全等价**（插哪个都行 ✓）—— 详见 [`connector-plan.md`](connector-plan.md) ✓。
